...
├── manifest.json           # Generated by import.py via generate_manifest.py
...
2. `import.py` invokes `generate_manifest.py` at the end of each run to produce `manifest.json` (with `papers` key)
3. `index.html` fetches `manifest.json` and displays articles + publications in separate tabs
...
`keywords`: 10-20 searchable free-form terms per article **and per paper**, extracted by Claude Haiku (model alias `KEYWORDS_MODEL` in `import.py`). Lowercase, mostly French with English technical terms preserved (DSPy, MoE, RAG, etc.). Not displayed in the UI — they only feed the search haystack in `index.html` so users can find content by technical terms, tools, people, architectures not captured in the 1-3 editorial `tags`. For papers, the input is the first 2 pages of the PDF truncated to ~3000 chars (abstract + intro) via `extract_pdf_abstract()`. Extraction runs by default during `import.py`; use `--no-keywords` to disable. Backfill script: `scripts/extract_keywords.py` (resumable, checkpoints every 10 items).
